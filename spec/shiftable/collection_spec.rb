  ...
  describe "with side effects" do
    ...
    context "with before_save" do
      ...
      it "does not change LaserBlasterRounds" do
        block_is_expected.to_not change { LaserBlasterRound.all.pluck(:space_federation_id).uniq }.from([shift_from.id])
      end
      it "does not change AlienBlasterRounds" do
        block_is_expected.to_not change { AlienBlasterRound.all.pluck(:space_federation_id).uniq }.from([shift_from.id])
      end
      it "does not change ThunderBlasterRounds" do
        block_is_expected.to_not change { ThunderBlasterRound.all.pluck(:space_federation_id).uniq }.from([shift_from.id])
      end
    end
  end
end
